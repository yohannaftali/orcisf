...
| #51 | fix(src): foreground-drawlist overlays (dock tab icons, joint/member/grid labels) bleed over the menu bar and neighboring panels | closed | 2026-08-16 |
...
| #58 | epic(src): FE analysis results visualization (deformed shape, force diagrams, tables) | closed (all 4 sub-issues #59-#62 closed, blocker #63 fixed, #61/#71/#62 each got dedicated live confirmation passes) | 2026-08-18 |
...
| #63 | bug(src): RunPanel hangs after reporting Converged, never completes (small dataset, worker_threads=15) | closed (tester: PASS, independently reproduced -- used different parameters than coder's own verification (worker_threads=1, Max generations=25 vs. coder's 15/40) against the same DatasetWithStr scratch dataset; Progress panel correctly showed "100% / Generation 25 of 25 / Run complete." immediately after the run, matching DebugLog's ~200ms completion timing; process confirmed Responding=True throughout, no regression to the in-progress display path per code review; see AGENTS.md's #63 section) | 2026-08-18 |
| #64 | docs(src): fix pre-existing "Nmm" mislabeling -- AM moment/torsion fields are actually N*m | closed (coder: implemented + verified -- all label sites fixed, .opt output confirmed value-identical, zero-warning build) | 2026-08-18 |
| #65 | bug(src): fak_plus=0/fak_kali=1 causes an out-of-bounds fitstr[-1] read in RunOptimization() | closed (coder: fixed at both GUI and engine layers, verified via orcisf_cli -- bad combo now fails with a clear error, valid combos unaffected) | 2026-08-18 |
| #66 | feat(src): load analysis results from saved .str file on Open Data | closed (tester: 4/4 PASS -- interactively confirmed live: Open Data loaded a real .str with zero Run clicks, Results panel's Support Reactions matched orcisf_cli's ANALYSIS_RESULTS exactly) | 2026-08-18 |
| #67 | epic(src): manual-dimension "Analyze" mode (design check without cost optimization) | closed (AC1/AC2 interactively confirmed PASS live; AC3 met now that #68/#69/#70 are closed) | 2026-08-18 |
| #68 | feat(src): engine analyze-only entry point (fixed-design analysis, no optimization search) | closed (tester: 4/4 PASS, independently re-verified with different dataset params/seed than coder's own test) | 2026-08-18 |
| #69 | feat(src): per-member manual dimension input panel for Analyze mode | closed (tester: 5/5 PASS -- interactively confirmed live: dropdown table, sensible defaults, Run Analyze button, dock-tab-icon reachability, no crash) | 2026-08-18 |
| #70 | feat(src): Analyze-mode design-check results display (demand vs. capacity, safe/unsafe) | closed (tester: 4/4 PASS -- interactively confirmed live: results table + "Unsafe: 8 of 8" summary + viewport turning red all matched; AC4's mixed-verdict scenario combines this session's live edit-and-rerun with the standalone undersized-vs-adequate test) | 2026-08-18 |
| #71 | feat(src): 3D-viewport force diagram overlay (N/V/M/T ribbons on the structure) | closed (tester: interactively CONFIRMED live -- filled colored diagram planes visible on real beams/columns, all 4 components simultaneously with distinct colors, Values labels showing correct magnitudes, Fill toggle working) | 2026-08-18 |
| #72 | fix(src): loads disappear from Loads panel after a completed optimization run | closed (tester: PASS -- both pre-run AND post-run states confirmed live: Loads panel showed batang 5-8 at 35000 N/m before Run, and identically 35000 N/m after a completed run showing "100%/Run complete.", screenshots for both) | 2026-08-18 |
